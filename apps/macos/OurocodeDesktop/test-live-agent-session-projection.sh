...
APP_ROOT="${0:A:h}"
FIXTURE_BINARY=$(mktemp "${TMPDIR:-/tmp}/ourocode-live-agent-session.XXXXXX")
trap 'rm -f "$FIXTURE_BINARY"' EXIT

swiftc -parse-as-library -warnings-as-errors \
  "$APP_ROOT/Sources/OurocodeDesktop/LiveAgentSessionProjectionPolicy.swift" \
  ...
  "$APP_ROOT/Tests/LiveAgentSessionProjectionFixture.swift" \
  -o "$FIXTURE_BINARY"
"$FIXTURE_BINARY"
