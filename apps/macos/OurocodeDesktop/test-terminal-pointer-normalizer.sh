...
APP_ROOT="${0:A:h}"
FIXTURE_BINARY=$(mktemp /private/tmp/ourocode-terminal-pointer.XXXXXX)
...
swiftc -warnings-as-errors -D OUROCODE_GHOSTTY_METAL_SURFACE \
  ...
  "$APP_ROOT/Sources/OurocodeDesktop/TerminalPointerNormalizer.swift" \
  "$APP_ROOT/Sources/OurocodeDesktop/TerminalScrollProjectionPolicy.swift" \
  "$APP_ROOT/Tests/TerminalPointerNormalizerFixture.swift" \
  -o "$FIXTURE_BINARY"
